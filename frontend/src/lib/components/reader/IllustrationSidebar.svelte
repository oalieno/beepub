<script lang="ts">
  ...
  function getStyleLabel(ill: IllustrationOut): string {
    ...
    if (ill.style_prompt) return STYLE_LABELS[ill.style_prompt] ?? ill.style_prompt;
    return 'Default';
  }
</script>
...
<div class="fixed right-0 top-0 bottom-0 z-50 w-80 max-w-[85vw] shadow-2xl flex flex-col {darkMode ? 'bg-gray-900 border-l border-gray-800' : 'bg-card border-l border-border'}">
  <div class="flex items-center justify-between px-4 py-3 border-b {darkMode ? 'border-gray-800' : 'border-border'}">
    <div class="flex items-center gap-2">
      <Sparkles size={14} class={darkMode ? 'text-purple-400' : 'text-purple-500'} />
      <p class="text-sm font-semibold {darkMode ? 'text-gray-200' : 'text-foreground'}">
        Illustrations
        {#if illustrations.length > 0}
          <span class="ml-1.5 text-xs font-normal {darkMode ? 'text-gray-500' : 'text-muted-foreground'}">{illustrations.length}</span>
        {/if}
      </p>
    </div>
    <button
  ...
  <div class="flex-1 overflow-y-auto p-2">
    {#if illustrations.length === 0}
      <p class="text-sm {darkMode ? 'text-gray-500' : 'text-muted-foreground'} py-4 text-center">No illustrations yet.</p>
    {:else}
      <div class="flex flex-col gap-1">
        {#each illustrations as ill (ill.id)}
          <div
            class="w-full text-left px-3 py-2.5 rounded-lg transition-colors group {ill.status === 'completed' ? 'cursor-pointer' : ''} {darkMode ? 'hover:bg-gray-800' : 'hover:bg-accent'}"
            role="button"
            tabindex="0"
            onclick={() => { if (ill.status === 'completed') onselect?.(ill); }}
            onkeydown={(e) => { if (e.key === 'Enter' && ill.status === 'completed') onselect?.(ill); }}
          >
            <div class="flex gap-2.5 items-start">
              <!-- Thumbnail or status indicator -->
              <div class="w-12 h-12 rounded-md flex-shrink-0 overflow-hidden {darkMode ? 'bg-gray-800' : 'bg-muted'} flex items-center justify-center">
                ...
                {:else if ill.status === 'generating'}
                  <Loader2 size={16} class="animate-spin {darkMode ? 'text-purple-400' : 'text-purple-500'}" />
                {:else if ill.status === 'failed'}
                  <AlertCircle size={16} class="text-red-400" />
                {:else}
                  <Loader2 size={16} class="animate-spin {darkMode ? 'text-gray-600' : 'text-muted-foreground'}" />
                {/if}
              </div>
                    ...
                    ill.status === 'completed'
                      ...
                      : ill.status === 'generating'
                        ...
                        : ill.status === 'failed'
                          ...
                          : darkMode ? 'bg-gray-800 text-gray-500' : 'bg-muted text-muted-foreground'
                  }">
                    {getStyleLabel(ill)}
                  </span>
                  {#if ill.status === 'generating'}
                    <span class="text-[10px] {darkMode ? 'text-purple-400' : 'text-purple-500'}">generating...</span>
                  {:else if ill.status === 'failed'}
                    <span class="text-[10px] text-red-400">failed</span>
                  {/if}
                </div>
                <p class="text-xs leading-snug {darkMode ? 'text-gray-300' : 'text-foreground'}">
                  {truncate(ill.text)}
                </p>
                <p class="text-[10px] mt-1 {darkMode ? 'text-gray-600' : 'text-muted-foreground/60'}">{formatDate(ill.created_at)}</p>
              </div>
              ...
              {#if ondelete}
                <button
                  ...
                  onclick={(e) => { e.stopPropagation(); ondelete?.(ill); }}
                >
                  <Trash2 size={13} />
                </button>
              {/if}
