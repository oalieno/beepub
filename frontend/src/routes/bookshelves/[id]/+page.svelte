<script lang="ts">
  ...
  let filteredAddBooks = $derived(addSearchQuery
...
<div class="max-w-6xl mx-auto px-4 sm:px-6 py-6">
  ...
  {:else if shelf}
    <div class="flex items-end justify-between mb-8">
      <div>
        <a href="/bookshelves" class="text-muted-foreground hover:text-foreground text-sm mb-1 inline-block">← Shelves</a>
    ...
    {#if books.length === 0}
      <div class="bg-card card-soft rounded-2xl p-12 text-center">
        ...
        <p class="text-muted-foreground/70 text-sm mt-1">Add some books to get started.</p>
      </div>
    {:else}
      <BookGrid {books} />
    {/if}
...
<Modal title="Add Books to Shelf" open={showAddModal} onclose={() => (showAddModal = false)}>
  <div class="space-y-3">
    <input
    ...
    <div class="max-h-80 overflow-y-auto space-y-2">
      {#each filteredAddBooks as book}
        ...
        >
          ...
          {:else}
            <div class="w-8 h-12 bg-muted rounded flex items-center justify-center text-muted-foreground text-xs">No cover</div>
          {/if}
          <div class="min-w-0">
            <p class="font-medium truncate text-foreground">{book.display_title ?? 'Untitled'}</p>
            <p class="text-muted-foreground text-xs truncate">{(book.display_authors ?? []).join(', ')}</p>
          </div>
        </button>
      {/each}
    </div>
