...
class Library(Base, TimestampMixin):
    ...
    accesses: Mapped[list["LibraryAccess"]] = relationship("LibraryAccess", back_populates="library")
    library_books: Mapped[list["LibraryBook"]] = relationship("LibraryBook", back_populates="library")


class LibraryAccess(Base):
    __tablename__ = "library_access"

    library_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("libraries.id", ondelete="CASCADE"), primary_key=True)
    ...
    user: Mapped["User"] = relationship("User", foreign_keys=[user_id], back_populates="library_accesses")
