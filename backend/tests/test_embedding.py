...
def _fake_response(vectors: list[list[float]], status_code: int = 200) -> MagicMock:
    """Build a mock httpx.Response with the given embedding vectors."""
    resp = MagicMock()
...
class TestEmbedTexts:
    ...
    async def test_returns_empty_list_for_empty_input(self):
        result = await embed_texts([], api_key="fake-key")
        assert result == []

    @pytest.mark.asyncio
    async def test_single_text_request_formation(self):
        raw_vector = [3.0, 4.0, 0.0]
        mock_post = AsyncMock(return_value=_fake_response([raw_vector]))
        ...
        with patch("app.services.embedding.httpx.AsyncClient", return_value=mock_client):
    ...
    async def test_batch_multiple_texts(self):
        ...
        mock_post = AsyncMock(return_value=_fake_response(raw_vectors))
    ...
    async def test_custom_model_and_dimensionality(self):
        mock_post = AsyncMock(return_value=_fake_response([[1.0, 0.0, 0.0, 0.0]]))
...
class TestEmbedText:
    ...
    async def test_returns_single_vector(self):
        ...
        with patch("app.services.embedding.httpx.AsyncClient", return_value=mock_client):
            result = await embed_text("single text", api_key="k")
    ...
    async def test_passes_model_and_dimensionality(self):
        mock_post = AsyncMock(return_value=_fake_response([[1.0]]))
